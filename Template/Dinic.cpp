...
struct FlowEdge {
  ...
  FlowEdge(int v, int u, long long cap,int id=-1) : v(v),
   u(u), cap(cap),id(id) {}
};
struct Dinic {
  vector<FlowEdge> edges; vector<vector<int> > adj;
  int n, m = 0; int s, t;
  vector<int> level, ptr,flow_through; queue<int> q; 
  vector<bool>vis; int maxid=0;
...
if (level[v] + 1 != level[u] || 
	edges[id].cap - edges[id].flow < 1) continue;
...
{
  map<pair<int,int>,bool>matched;vector<vector<int> >adj;
